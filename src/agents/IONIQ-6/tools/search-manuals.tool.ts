...
async function searchAllManuals(query: string): Promise<string> {
    ...
    if (topResults.length === 0) {
        return JSON.stringify({
            found: false,
            message: "No matching content found in the IONIQ 6 manuals for this query.",
        });
    }
...
export const searchManuals = tool(
    async ({ query }) => {
        console.log(`${LogColors.BRIGHT_CYAN}[search_ioniq6_manuals]${LogColors.RESET} INPUT: query="${query}"`);
        const result = await searchAllManuals(query);
        const parsed = JSON.parse(result);
        console.log(
            `${LogColors.BRIGHT_CYAN}[search_ioniq6_manuals]${LogColors.RESET} OUTPUT: found=${parsed.found}, count=${parsed.count ?? 0}`
        );
        return result;
    },
    {
        name: "search_ioniq6_manuals",
        description:
            "Search through all Hyundai IONIQ 6 car manuals (PDF documents) for content matching the given query. Returns relevant excerpts with source file name and page number. Use this tool to find information about the IONIQ 6 — features, maintenance, controls, specifications, safety, etc.",
        schema: z.object({
            query: z
                .string()
                .describe(
