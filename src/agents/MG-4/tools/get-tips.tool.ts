...
async function getTipsFromManuals(topic?: string): Promise<string> {
    ...
    if (selected.length === 0) {
        return JSON.stringify({
            found: false,
            message: topic
                ? `No content found in the MG-4 manuals related to "${topic}" for generating tips.`
                : "No content found in the MG-4 manuals for generating tips.",
        });
    }
    ...
    return JSON.stringify({
        ...
        instruction: "Based on the following excerpts from the MG-4 car manuals, generate helpful and practical tips for the car owner. Focus on features, abilities, useful shortcuts, safety advice, and things that are good to know.",
...
export const getTips = tool(
    async ({ topic }) => {
        console.log(`${LogColors.BRIGHT_BLACK}[get_mg4_tips]${LogColors.RESET} INPUT: topic="${topic || "(general)"}"`);
        const result = await getTipsFromManuals(topic || undefined);
        const parsed = JSON.parse(result);
        console.log(
            `${LogColors.BRIGHT_BLACK}[get_mg4_tips]${LogColors.RESET} OUTPUT: found=${parsed.found}, count=${parsed.count ?? 0}`
        );
        return result;
    },
    {
